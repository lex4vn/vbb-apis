...
class LoginGoogleAndroidAction extends CAction
{
    public function run()
    {
        header('Content-type: application/json');
        $accessToken = isset($_GET['accessToken']) ? $_GET['accessToken'] : null;
        $id_token = isset($_GET['id_token']) ? $_GET['id_token'] : null;
        $token_type = 'Bearer';
        $expires_in = '3600';
        if ($accessToken == null) {
        ...
        if ($id_token == null) {
            echo json_encode(array('code' => 5, 'message' => 'Missing params id_token'));
            return;
        }
        // Get user infomation
        ...
        $arr = json_encode(array('access_token' => $accessToken, 'token_type' => $token_type, 'expires_in' => $expires_in, 'id_token' => $id_token));
        if (isset($arr)) {
            $client->setAccessToken($arr);
        }
